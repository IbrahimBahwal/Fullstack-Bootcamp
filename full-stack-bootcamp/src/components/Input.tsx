...
const Input = () => {
  return (
        ...
        <div className="relative w-full max-w-4xl">
            ...
            <button
            className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-transparent  hover:brightness-105"
            >
            <img src={StarIcon} alt="Star Icon" className="w-8 h-8" />
            </button>
        </div>

        <button className="ml-4 px-4 py-4 rounded-full bg-(--bg-dark) bg-opacity-20 backdrop-blur-[3px] border border-(--gold-cream) text-amber-100 hover:border-(--gold-cream)"><img src={AddIcon} alt="Add Icon" className="w-5 h-5" /></button>
        </div>
  );
};
